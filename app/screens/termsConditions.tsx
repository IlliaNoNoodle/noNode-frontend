...
import { checkAuthAtom, privacyPolicyAcceptedAtom, termsConditionsAcceptedAtom } from '../store/index';
...
const TermsConditions: React.FC<PrivacyPolicyProps> = ({
  ...
  const [, setIsAuthenticated] = useAtom(checkAuthAtom);
  const [privacyPolicyAccepted] = useAtom(privacyPolicyAcceptedAtom);
  const [, setTermsAccepted] = useAtom(termsConditionsAcceptedAtom);
  ...
  interface TriangleBackArrowProps {
    onPress?: () => void;
    size?: number;
    color?: string;
  }

  const TriangleBackArrow: React.FC<TriangleBackArrowProps> = ({ onPress, size = 24, color = "white" }) => (
  <TouchableOpacity
    onPress={() => {
